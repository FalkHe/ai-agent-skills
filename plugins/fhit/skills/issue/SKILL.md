...
Comment on the issue: proposals as a numbered list in the product owner's language, each "reply `Dn: <your answer>` to change", plus `approve` / `stop`.
...
## 7. Report
Final comment: what the product can now do, with the PR links. Name anything that failed as a user would notice it.
